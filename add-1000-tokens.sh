...
TYPES=("tokens" "assets" "balances" "accounts" "wallets" "vaults" "reserves" "pools" "funds" "treasuries")
...
for i in {1..1000}; do
  ...
  cat > $PROJECT/contracts/$TYPE/token-$NUM.clar << EOF
(define-public (token-$NUM (amount uint))
  (ok (- amount u$NUM)))
EOF
  
  git add . && git commit -m "feat($PROJECT): add $TYPE token $NUM" --quiet
